...
csv_path = '/my-path-csv/file.csv'
json_output_file = '/path-out/file.json'
model_name = 'app.ModelName'


def read_csv_write_json(csv_file, json_file, appModel):
    csv_row = []
    with open(csv_file) as csv_f:
        ...
        for row in csv_r:
            id = id + 1
            csv_row.extend(
                [{"model": appModel, "pk": id, "fields": {field[i]: row[field[i]] for i in range(len(field))}}])
